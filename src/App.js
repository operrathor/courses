...
export default function App() {
  ...
  useEffect(() => {
    const storedCourses = JSON.parse(localStorage.getItem(LOCAL_STORAGE_COURSES_KEY))
    if (storedCourses && storedCourses.length > 0) {
      ...
      (async () => {
        const initialLoadData = await Promise.all(initialLoad.map(async ({id, color}) => await newCourse(id, color)))
  ...
  function toggleGroup(courseId, enabledGroups) {
    const newCourses = [...courses]
    newCourses.find((course) => course.id === courseId).enabledGroups = enabledGroups
  ...
  async function fetchCourse(id) {
    const data = await fetch(`http://localhost:3001/courses/${id}`).then(res => res.json())
    return {
      course: {
        id: id,
        title: data.title,
  ...
  async function newCourse(id, color = colorPalette[Math.floor(Math.random() * colorPalette.length)]) {
    ...
    <>
      ...
      <div className='container'>
        ...
        <aside className='sidebar'>
          <h1>Courses</h1>
          <Courses courses={courses} onGroupToggle={toggleGroup} onColorChange={changeColor} onRemove={removeCourse} />
